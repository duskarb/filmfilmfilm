...
export function MovieDetail({ movieId, onMovieSelect, starredIds, toggleStar, contentOverrides, onContentSave }: MovieDetailProps) {
    ...
    <div className="flex flex-col items-center w-full pt-4 pb-32">
      ...
      <div className="w-full max-w-3xl px-margin-page flex flex-col">
        ...
        <article
          ...
          className={`prose prose-zinc dark:prose-invert prose-lg md:prose-xl max-w-none [&_p]:text-black dark:[&_p]:text-zinc-100 leading-[1.8] font-sans font-normal ${editState === 'idle' ? 'cursor-text' : ''}`}
        >
          {editState === 'idle' ? (
            <>
              {displayContent ? (
                ...
                <p className="text-zinc-500 italic not-prose">
                  ...
                  <button onClick={enterEdit} className="underline hover:text-black dark:hover:text-white transition-colors">
                    Add one
                  </button>
                </p>
              )}
