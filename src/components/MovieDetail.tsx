  ...
  return (
    <>
      ...
      <aside className="w-full md:w-1/3 flex-shrink-0">
        <div className="md:sticky md:top-40 flex flex-col gap-8">
          <div className="flex flex-col gap-2">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold tracking-tighter leading-[1.1] uppercase text-white">
              {movie?.title}
            </h1>
          </div>
          ...
          <div className="flex flex-col gap-6 border-t border-zinc-800 pt-6">
            <div className="flex flex-col">
              <span className="font-mono text-[10px] tracking-widest uppercase text-zinc-500 mb-1">Director</span>
              <span className="text-body-lg font-medium text-zinc-300">{movie?.director}</span>
            </div>
            <div className="flex flex-col">
              <span className="font-mono text-[10px] tracking-widest uppercase text-zinc-500 mb-1">Year</span>
              <span className="text-body-lg font-medium text-zinc-300">{movie?.year}</span>
            </div>
          </div>

          <div className="border-t border-zinc-800 pt-6 mt-2">
            <button onClick={() => alert('Saved to collection!')} className="flex items-center gap-2 group hover:text-white transition-colors text-zinc-500 font-bold uppercase tracking-widest text-[12px]">
              <span className="group-hover:text-white">Save to Collection</span>
              <Bookmark size={14} className="group-hover:fill-current" />
            </button>
            <button onClick={() => onMovieSelect('')} className="mt-4 flex items-center gap-2 group hover:text-white transition-colors text-zinc-500 font-bold uppercase tracking-widest text-[12px]">
              <span className="group-hover:text-white">← Back to Index</span>
            </button>
          </div>
        </div>
      </aside>
      ...
      <main className="w-full md:w-2/3 flex flex-col gap-12 pb-20">
        <div className="w-full aspect-[16/9] overflow-hidden bg-zinc-900 shadow-2xl rounded-sm">
          <motion.img 
            key={movie?.id}
        ...
        <article className="max-w-none text-lg leading-relaxed markdown-content text-zinc-400">
          {movie?.content ? (
